...
void AmigaLoader::RenderScrollText(float _frameTime)
{
	...
	for (int i = 0; i < charsPerScreen; ++i)
	{
		...
		float y = 40.0f + sin(ourPhase) * 20.0f;
		int colour = sin(ourPhase * 4.0f) * 100.0f + 101.0f;
		RGBAColour col;
		col.Set(32, 64, 64);
		glColor3ubv(col.GetData());
		g_editorFont.DrawText2DSimple(x, y, textHeight + 8, buf);
...
void AmigaLoader::RenderLogo(float _frameTime)
{
	...
	posOffset.x = sinf(logoPhase * 0.6f) * 40.0f;
	posOffset.y = sinf(logoPhase * 0.4f + 2.0f) * 40.0f;

	pos1.x = 80.0f + sinf(logoPhase) * 30.0f		+ posOffset.x;
	pos1.y = 120.0f + sinf(logoPhase + 1) * 40.0f	+ posOffset.y;

	pos2.x = 80.0f + sinf(logoPhase+1.5) * 40.0f	+ posOffset.x;
	pos2.y = 120.0f + sinf(logoPhase+3.5) * 50.0f	+ posOffset.y;
