...
void TaskManagerInterfaceGestures::AdvanceGestures()
{
		 ...
		 !g_inputManager.controlEvent( ControlGestureActive ) )
    {
        ...
        if( g_app->m_gesture->GestureSuccess() )
        {
            ...
            int taskType = g_app->m_taskManager->MapGestureToTask( symbolId );
...
void TaskManagerInterfaceGestures::AdvanceScrolling()
{
    if( m_screenX != m_desiredScreenX )
    {
        ...
        if( fabs( m_desiredScreenX - m_screenX ) < 0.001f )
        {
...
void TaskManagerInterfaceGestures::RenderTaskManager()
{
    ...
    if( fabs(m_screenX) < 0.1f )
    {
        ...
        ScreenZone *zoneResearch    = new ScreenZone( "ShowScreen", LANGUAGEPHRASE("newcontrols_showresearch"),    screenCentre-boxW*1.5f, boxY, boxW, boxH, 3 );
...
void TaskManagerInterfaceGestures::RenderCompass( float _screenX, float _screenY, Vector3 const &_worldPos, bool _selected )
{
    ...
    glBegin( GL_QUADS );
        ...
        glTexCoord2i(1,1);      glVertex2fv( (screenPos + compassRight * compassSize + compassVector * compassSize).GetData() );
        glTexCoord2i(1,0);      glVertex2fv( (screenPos + compassRight * compassSize - compassVector * compassSize).GetData() );
        glTexCoord2i(0,0);      glVertex2fv( (screenPos - compassRight * compassSize - compassVector * compassSize).GetData() );
    glEnd();
